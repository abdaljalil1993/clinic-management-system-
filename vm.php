...
<body style="background-color: #cfcfcf"><div class="container">
				<h2 style="text-align: center; color: rgb(255, 255, 255);">Zoom SECTION</h2>

<?php ob_start();
	include'conn.php';

$run1=mysqli_query($conn,"select * from pat where pname='".$_COOKIE['name']."'and ppass='".$_COOKIE['pass']."'");
$row2=mysqli_fetch_array($run1);

	$run=mysqli_query($conn,"select * from links where pid='".$row2['pid']."'");
  if(mysqli_num_rows($run)>0){
	...
	{ 
		...
		?>
			...
			<div class="vr">
				<h2 style="text-align: center; color: white;" class="animated flash infinite">Link Description : <?php echo $row['des'];?></h2>

				<p style="color: white;font-size: 21px;font-weight: bold; text-align: center;font-family: tahoma;">Link  : <br><?php if( !empty($row['link'])) echo $row['link'];
						if($row['link']==' '){
							echo'no meeting';
						}
				?></p>
  ...
  echo '<div class="container">
      ...
      <h2 class="text-white text-center">no Meeting for you yet  </h2>
